...
from soc_playbook_router.executor import build_executor
from soc_playbook_router.selectors import build_retrieval_selector, build_rule_selector, build_hybrid_selector
...
def _run(name: str, selector, threshold: float) -> dict:
    ...
    runtime = KVRMRuntime(registry, selector, validator, executor, threshold=threshold, fallback_action_id='request_human_triage')
    runner = BenchmarkRunner(runtime, REGISTRY_PATH, CASES_PATH, OUTPUT_DIR / name)
    return runner.run(run_name=name)


def main() -> None:
    retrieval_result = _run('soc_playbook_router_retrieval', build_retrieval_selector(TRAIN_CASES_PATH), threshold=0.60)
    rule_result = _run('soc_playbook_router_rule', build_rule_selector(), threshold=0.60)
    hybrid_result = _run('soc_playbook_router_hybrid', build_hybrid_selector(TRAIN_CASES_PATH), threshold=0.60)
