...
from medical_workflow_router import build_hybrid_selector, build_retrieval_selector
...
def test_medical_benchmark_runs_and_writes_artifacts(tmp_path):
    ...
    result = BenchmarkRunner(runtime, REGISTRY_PATH, CASES_PATH, tmp_path).run(run_name="medical_test")
    assert result["metrics"]["structural_validity_rate"] == 1.0
    assert result["metrics"]["unsupported_case_rejection_rate"] >= 1.0


def test_medical_hybrid_beats_retrieval_on_supported_accuracy(tmp_path):
    registry = load_registry(REGISTRY_PATH)
