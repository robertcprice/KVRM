...
from drone_mission_router import build_retrieval_selector, build_hybrid_selector
...
def test_drone_benchmark_runs_and_writes_artifacts(tmp_path):
    registry = load_registry(REGISTRY_PATH)
    runtime = KVRMRuntime(registry, build_retrieval_selector(TRAIN_CASES_PATH), DeterministicValidator(registry), build_executor(), threshold=0.60, fallback_action_id='manual_handoff')
    result = BenchmarkRunner(runtime, REGISTRY_PATH, CASES_PATH, tmp_path).run(run_name='drone_test')
...
def test_hybrid_beats_retrieval_on_supported_accuracy(tmp_path):
    registry = load_registry(REGISTRY_PATH)
    retrieval_runtime = KVRMRuntime(registry, build_retrieval_selector(TRAIN_CASES_PATH), DeterministicValidator(registry), build_executor(), threshold=0.60, fallback_action_id='manual_handoff')
    hybrid_runtime = KVRMRuntime(registry, build_hybrid_selector(TRAIN_CASES_PATH), DeterministicValidator(registry), build_executor(), threshold=0.60, fallback_action_id='manual_handoff')
