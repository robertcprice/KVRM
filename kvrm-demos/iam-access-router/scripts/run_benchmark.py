...
def main() -> None:
    retrieval_result = _run("iam_access_router_retrieval", build_retrieval_selector(TRAIN_CASES_PATH), 0.60)
    rule_result = _run("iam_access_router_rule", build_rule_selector(), 0.60)
    hybrid_result = _run("iam_access_router_hybrid", build_hybrid_selector(TRAIN_CASES_PATH), 0.60)
