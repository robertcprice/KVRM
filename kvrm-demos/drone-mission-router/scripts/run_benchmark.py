...
from drone_mission_router.executor import build_executor
from drone_mission_router.selectors import build_retrieval_selector, build_rule_selector, build_hybrid_selector
...
def _run(name: str, selector, threshold: float) -> dict:
    ...
    runtime = KVRMRuntime(registry, selector, DeterministicValidator(registry), build_executor(), threshold=threshold, fallback_action_id='manual_handoff')
    return BenchmarkRunner(runtime, REGISTRY_PATH, CASES_PATH, OUTPUT_DIR / name).run(run_name=name)


def main() -> None:
    retrieval_result = _run('drone_mission_router_retrieval', build_retrieval_selector(TRAIN_CASES_PATH), 0.60)
    rule_result = _run('drone_mission_router_rule', build_rule_selector(), 0.60)
    hybrid_result = _run('drone_mission_router_hybrid', build_hybrid_selector(TRAIN_CASES_PATH), 0.60)
